...
if (!empty($_POST)) {
  ...
  $sql = "INSERT INTO packing_lists (ultimo_nro_revision,id_tarea) VALUES ($nro_revision,?)";
  $q = $pdo->prepare($sql);
  $q->execute([$idTarea]);
  
  $id_packing_list = $pdo->lastInsertId();
  ...
  $sql = "SELECT count(id) cant FROM packing_lists_revisiones where descripcion = 'Emision original' and id_proyecto = ?";
  $q = $pdo->prepare($sql);
  $q->execute([$_POST['id_proyecto']]);
  $data = $q->fetch(PDO::FETCH_ASSOC);
  $nroPL = $data['cant']+1;
  ...
  $sql = "INSERT INTO packing_lists_revisiones (id_packing_list, id_proyecto, nombre, fecha, id_usuario, id_estado_packing_list, nro_revision, descripcion, anulado, numero,`id_cuenta_realizo`, `id_cuenta_reviso`, `id_cuenta_valido`) VALUES (?,?,?,?,?,1,$nro_revision,?,0,?,?,?,?)";
  $q = $pdo->prepare($sql);
  $q->execute([$id_packing_list,$_POST['id_proyecto'],$_POST['nombre'],$_POST['fecha'],$_SESSION['user']['id'],$descripcion,$nroPL,$idCuentaRealizo,$idCuentaReviso,$idCuentaValido]);
  ...
  $q->execute(array($_SESSION['user']['id']));
	...
	$sql = " select t.id_usuario,u.email from usuarios_tipos_notificacion t inner join usuarios u on u.id = t.id_usuario where t.id_tipo_notificacion = 7 ";
	foreach ($pdo->query($sql) as $row) {
		
		$sql = "INSERT INTO `notificaciones`(`id_tipo_notificacion`, `id_usuario`, `fecha_hora`, `leida`,detalle,id_entidad) VALUES (7,?,now(),0,?,?)";
		$q = $pdo->prepare($sql);
		$q->execute([$row[0],'ID PackingList: #'.$id_packing_list_revision,$id_packing_list_revision]);
		
		$address = $row[1];
		
		$titulo = "ERP Notificaciones - Módulo Ingeniería - Nueva Packing List (".$descripcionProyecto.")";
		$mensaje="Nueva packing list dada de alta en el sistema: #".$id_packing_list_revision;
                      ...
                      <div class="row">
                        <div class="col">
                          <div class="form-group row">
                            ...
                            <div class="col-sm-9"><input name="fecha" type="date" autofocus onfocus="this.showPicker()" class="form-control" required="required" value=""></div>
                          </div>
						  <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Nombre PL(*)</label>
                            <div class="col-sm-9"><input name="nombre" type="text" class="form-control" required="required" value=""></div>
                          </div>
                          <div class="form-group row">
