<?php
    ...
    if (!empty($_POST)) {
		...
		$qSigno = $pdo->prepare("SELECT signo_cpr FROM `regimenes_facturacion` WHERE id = ?");
		$qSigno->execute([$_POST['id_regimen']]);
		$signoCpr = (int)$qSigno->fetchColumn();
		$monto = floatval($_POST['monto']);
		$montoSignado = $signoCpr === 2 ? -abs($monto) : abs($monto);

		$sql = "INSERT INTO `facturas_compra_retenciones`(`id_factura_compra`, `regimen_text`, `codigo`, `articulo`, `monto`, `porcentaje`, `base_imponible`)
		...
		if (!empty($_POST['btn2'])) {
			...
			header("Location: nuevaRetencionFacturaCompra.php?id=".$_GET['id']);
		}
  ...
  <body>
    ...
    <?php include('menu.php');?>
        ...
        <div class="page-body"><?php
          $ubicacion="Nuevo Ítem Retención de Factura Compra";
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm-12">
				...
				<form class="form theme-form" role="form" method="post" action="nuevaRetencionFacturaCompra.php?id=<?php echo $_GET['id'];?>">
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
							...
							<table class="display" id="dataTables-example667">
								...
								<tbody>
								  <?php
									$pdo = Database::connect();
									if (!empty($_GET['id'])) {
										$sql = " SELECT r.`id`, COALESCE(r.`regimen_text`, r.`codigo`, r.`articulo`, ''), r.`monto` FROM `facturas_compra_retenciones` r WHERE r.`id_factura_compra` = ".$_GET['id'];
										foreach ($pdo->query($sql) as $row) {
											...
											echo '<td><a href="eliminarRetencionFacturaCompra.php?id='. $row[0] . '&fc='. $_GET['id'] . '&monto='. $row[2] . '"><img src="img/icon_baja.png" width="24" height="25" border="0" alt="Eliminar" title="Eliminar"></a></td>';
											echo '</tr>';
										}
									}
