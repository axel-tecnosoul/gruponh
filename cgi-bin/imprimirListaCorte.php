                    ...
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
							...
							<b>Elaboró:</b> <?php echo $data['cuenta_elaboro'];?><br>
							<b>Verificó:</b> <?php echo $data['cuenta_verifico'];?><br>
							<b>Aprobó:</b> <?php echo $data['cuenta_aprobo'];?><br>
							</div>
							</div>
						<div class="row">
							<div class="col-11">
							...
							<tbody>
							  <?php
								...
								foreach ($pdo->query($sql) as $row) {
									...
									<tbody>
									<?php
									$sql2 = "SELECT lcp.`id`, m.concepto, lcp.`posicion`, lcp.`cantidad`, lcp.`largo`, lcp.`ancho`, lcp.`marca`, lcp.`peso`, lcp.`finalizado`, c.nro_colada, lcp.`diametro`, lcp.`calidad` FROM `lista_corte_posiciones` lcp inner join materiales m on m.id = lcp.`id_material` left join coladas c on c.id = lcp.`id_colada`  WHERE lcp.`id_lista_corte_conjunto` = ".$row[0];
							...
							Peso total de la lista: <?php echo number_format($pesoTotal,2);?>kgs
                        </div>
                      </div>
                    </div>
                </div>
