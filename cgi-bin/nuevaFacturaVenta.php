<?php
    ...
    if (!empty($_POST)) {
		...
		if(isset($_POST["id_certificado_avance_detalle"])){
			$ex=explode(",",$_POST["id_certificado_avance_detalle"]);
			foreach ($ex as $key => $id) {
				$sql = "UPDATE certificados_avances_detalle SET id_comprobante = ? WHERE id = ?";
				$q = $pdo->prepare($sql);
				$q->execute([$idFactura, $id]);
			}
		}
                      ...
                      <div class="row">
                        ...
                        if(isset($_GET["id"])){?>
                          <input type="hidden" name="id_certificado_avance_detalle" value="<?=$_GET["id"]?>"><?php
							...
							<div class="col-sm-9"><input name="numero" id="customInput" oninput="applyMask(this)" placeholder="1234-12345678" type="text" maxlength="99" class="form-control" required="required"></div>
							</div>
							<div class="form-group row">
