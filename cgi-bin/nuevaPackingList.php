...
if (!empty($_POST)) {
  ...
  $sql = "SELECT count(id) cant FROM packing_lists_revisiones where id_proyecto = ?";
  $q = $pdo->prepare($sql);
  $q->execute([$_POST['id_proyecto']]);
  $data = $q->fetch(PDO::FETCH_ASSOC);
  $nroPL = $data['cant']+1;

  $sql = "INSERT INTO packing_lists_revisiones (id_packing_list, id_proyecto, nombre, fecha, id_usuario, id_estado_packing_list, nro_revision, descripcion, anulado, numero) VALUES (?,?,?,?,?,1,$nro_revision,?,0,?)";
  $q = $pdo->prepare($sql);
  $q->execute([$id_packing_list,$_POST['id_proyecto'],$_POST['nombre'],$_POST['fecha'],$_SESSION['user']['id'],$descripcion,$nroPL]);
  ...
  $q->execute(array($_SESSION['user']['id']));
	...
	$titulo = "ERP Notificaciones - Módulo Ingeniería - Nueva Packing List";
