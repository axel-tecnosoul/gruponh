<?php
    ...
    if (!empty($_POST)) {
        ...
        $sql = "UPDATE `packing_lists_componentes` set `id_conjunto_lista_corte`= ?, `id_concepto`= ?, `cantidad`= ?, `observaciones`= ?, `id_estado_componente_packing_list`= ? where id = ?";
        $q = $pdo->prepare($sql);
		...
		$sql = "select plr.id_proyecto,pls.observaciones from packing_lists_secciones pls inner join packing_lists_revisiones plr on plr.id = pls.id_packing_list_revision where pls.id = ".$data['id_packing_list_seccion'];
		$q = $pdo->prepare($sql);
		$q->execute();
		$data2 = $q->fetch(PDO::FETCH_ASSOC);
		$idProyecto=$data2['id_proyecto'];
		$nombreSeccion=$data2['observaciones'];
    ...
    <div class="page-wrapper">
      ...
      <div class="page-body-wrapper">
        ...
        <div class="page-body"><?php
          $ubicacion="Modificar Componente de Sección: ".nombreSeccion;
                    ...
                    <div class="card-body">
                      <div class="row">
						...
						<select name="id_conjunto_lista_corte" id="id_conjunto_lista_corte" class="js-example-basic-single col-sm-12" onchange="jsCompletarCant(this.value);">
                              ...
                              $pdo = Database::connect();
                              $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                              $sqlZon = "SELECT lcc.`id`, lcc.`nombre`, lcc.`cantidad` FROM `listas_corte_conjuntos` lcc inner join listas_corte_revisiones lcr on lcr.id = lcc.id_lista_corte inner join proyectos p on p.id = lcr.id_proyecto WHERE lcc.`id_estado_lista_corte_conjuntos` = 4 and lcr.id_proyecto = ".$idProyecto;
                              $q = $pdo->prepare($sqlZon);
                              $q->execute();
                              while ($fila = $q->fetch(PDO::FETCH_ASSOC)) {
                                echo "<option value='".$fila['id']."'";
