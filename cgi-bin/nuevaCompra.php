<?php
    ...
    if (!empty($_POST)) {
		...
		$sql = "INSERT INTO `compras`(`id_pedido`, `id_cuenta_proveedor`, `fecha_emision`, `fecha_entrega`, `id_forma_pago`, `id_estado_compra`, `nro_oc`, `total`, `comentarios`, `id_moneda`, `tipo_cambio_dia`) VALUES (?,?,?,?,?,1,?,0,?,?,?)";
		$q = $pdo->prepare($sql);		   
		$q->execute([$_GET['idPedido'],$_POST['id_cuenta_proveedor'],$_POST['fecha_emision'],$_POST['fecha_entrega'],$_POST['id_forma_pago'],'',$_POST['comentarios'],$_POST['id_moneda'],$_POST['tipo_cambio_dia']]);
        
		$id = $pdo->lastInsertId();
		
		$sql = "INSERT INTO `compras_revisiones` (`id_compra`, `nro_revision`, `fecha_hora`, `comentarios`, `id_usuario`) VALUES (?,0,now(),'Revisión Inicial',?)";
		$q = $pdo->prepare($sql);		   
                    ...
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
							...
							<div class="col-sm-9"><input name="nro_oc" type="text" maxlength="99" class="form-control" required="required"></div>
							</div>
							-->
							<div class="form-group row">
							...
							<div class="col-sm-9"><input name="fecha_entrega" type="date" onfocus="this.showPicker()" value="" class="form-control"></div>
							</div>
							<div class="form-group row">
