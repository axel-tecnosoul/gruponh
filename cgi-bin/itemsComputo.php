    ...
    if (!empty($_POST)) {
		...
		if (empty($data)) {
			if ($_GET['modo'] == "nuevo") {
				...
				$sql = "select id,id_tarea,fecha,id_cuenta_solicitante,nro FROM `computos` where id = ?";
				$q = $pdo->prepare($sql);
				$q->execute([$id]);
				...
				$sql = "insert into `computos` (`nro_revision`, `id_tarea`, `fecha`, `id_cuenta_solicitante`, `id_estado`, `nro_computo`, `comentarios_revision`, `fecha_hora_revision`,nro) values (?,?,?,?,?,?,?,now(),?)";
				$q = $pdo->prepare($sql);
				$q->execute([$nro,$dataC['id_tarea'],$dataC['fecha'],$dataC['id_cuenta_solicitante'],$estadoComputo,$dataC['id'],$_POST['observaciones'],$dataC['nro']]);
				
				$idNuevoComputo = $pdo->lastInsertId();
				$sqlList = " SELECT `id_material`, `cantidad`, `fecha_necesidad`, `aprobado`, `reservado`, `comprado`, `cancelado`,comentarios FROM `computos_detalle` WHERE `id_computo` = ".$id;
                foreach ($pdo->query($sqlList) as $row) {
					...
					$q = $pdo->prepare($sql);
					$q->execute([$idNuevoComputo,$row[0],$row[1],$row[2],$row[3],$row[4],$row[5],$row[6],$row[7]]);
				}
				$sql = "INSERT INTO `computos_detalle`(`id_computo`, `id_material`, `cantidad`, `fecha_necesidad`, `aprobado`, `comentarios`) VALUES (?,?,?,?,0,?)";
				$q = $pdo->prepare($sql);
				$q->execute([$idNuevoComputo,$_POST['id_material'],$_POST['cantidad'],$_POST['fecha_necesidad'],$_POST['comentarios']]);
            ...
            <div class="row">
              <div class="col-sm-12">
                <div class="card">
				  ...
				  <form class="form theme-form" role="form" method="post" action="itemsComputo.php?modo=<?php echo $_GET['modo']?>&id=<?php echo $id?>">
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
							...
							<table class="display" id="dataTables-example667">
								...
								<tbody>
								  <?php
									$pdo = Database::connect();
									$sql = " SELECT d.`id`, m.`concepto`, d.`cantidad`, date_format(d.`fecha_necesidad`,'%d/%m/%y'), d.`aprobado`, d.`comentarios` FROM `computos_detalle` d inner join materiales m on m.id = d.id_material WHERE cancelado = 0 and d.id_computo = ".$_GET['id'];
                    ...
                    <div class="card-footer">
                      <div class="col-sm-9 offset-sm-3">
						...
						<button class="btn btn-primary" type="submit" value="2" name="btn2">Crear y Enviar a Aprobación</button>
