<?php
    ...
    if (!empty($_POST)) {
		...
		$sql = "INSERT INTO `facturas_compra_detalle`(`id_factura_compra`, `id_concepto_contable`, `cantidad`, `precio`, `subtotal`) VALUES (?,?,?,?,?)";
		$q = $pdo->prepare($sql);		   
		...
		$sql = " SELECT `cantidad`, `precio`, `subtotal` FROM `facturas_compra_detalle` WHERE `id_factura_compra` = ".$_GET['id'];    
		...
		$sql = "update `facturas_compra` set  `subtotal_gravado` = ?, `subtotal_no_gravado` = ?, `otros` = ?, `iva` = ?, `total` = ? where id = ?";
		$q = $pdo->prepare($sql);		   
		...
		$sql = "INSERT INTO logs(`fecha_hora`, `id_usuario`, `detalle_accion`,`modulo`,link) VALUES (now(),?,'Nuevo Ítem Detalle de Factura de Compra','Facturas de Compra','verCompra.php?id=$id')";
		$q = $pdo->prepare($sql);
  ...
  <body>
    ...
    <?php include('menu.php');?>
        ...
        <div class="page-body"><?php
          $ubicacion="Nuevo Ítem Detalle de Factura Compra";
