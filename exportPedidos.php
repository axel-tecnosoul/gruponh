					...
					<thead>
		             ...
		             <tbody>
		              <?php     
						...
						$sql = " SELECT pe.`id`, s.nombre, p.nombre, m.`concepto`, pd.`cantidad`, pd.`comprado`, um.`unidad_medida`, pe.`fecha`, pd.`fecha_necesidad`, pe.`aprobado`,ep.estado  FROM pedidos_detalle pd inner join pedidos pe on pe.id = pd.id_pedido inner join estados_pedidos ep on ep.id = pe.id_estado inner join `computos` c on c.id = pe.id_computo inner join tareas t on t.id = c.id_tarea inner join proyectos p on p.id = t.id_proyecto inner join sitios s on s.id = p.id_sitio inner join materiales m on m.id = pd.id_material inner join unidades_medida um on um.id = pd.id_unidad_medida WHERE 1 ";
					   ...
					   $sql = " SELECT pe.`id`, s.nombre, p.nombre, m.`concepto`, pd.`cantidad`, pd.`comprado`, um.`unidad_medida`, pe.`fecha`, pd.`fecha_necesidad`, pe.`aprobado`,ep.estado FROM pedidos_detalle pd inner join pedidos pe on pe.id = pd.id_pedido inner join estados_pedidos ep on ep.id = pe.id_estado  inner join proyectos p on p.id = pe.id_proyecto inner join sitios s on s.id = p.id_sitio inner join materiales m on m.id = pd.id_material inner join unidades_medida um on um.id = pd.id_unidad_medida WHERE 1 ";
                        foreach ($pdo->query($sql) as $row) {
