<?php
    ...
    if (!empty($_POST)) {
		...
		$sql = "UPDATE `sitios` set `nombre` = ?, id_cuenta_duenio = ?, `direccion` = ?, `latitud` = ?, `longitud` = ?, `observaciones` = ?, `id_pais` = ?, `id_empresa` = ?, `id_provincia` = ?, `id_localidad` = ?, `id_tipo_estructura` = ?, `altura` = ?, `ancho_cara` = ?, `peso_estructura` = ?, `id_tipo_montaje` = ?, `paso` = ?, `beta` = ?, `rugosidad` = ? where id = ?";
        $q = $pdo->prepare($sql);
        $q->execute([$_POST['nombre'],$_POST['id_cuenta_duenio'],$_POST['direccion'],$_POST['latitud'],$_POST['longitud'],$_POST['observaciones'],$_POST['id_pais'],$_POST['id_empresa'],$_POST['id_provincia'],$_POST['id_localidad'],$_POST['id_tipo_estructura'],$_POST['altura'],$_POST['ancho_cara'],$_POST['peso_estructura'],$_POST['id_tipo_montaje'],$_POST['paso'],$_POST['beta'],$_POST['rugosidad'],$_GET['id']]);

		$sql = "INSERT INTO logs(`fecha_hora`, `id_usuario`, `detalle_accion`,`modulo`,link) VALUES (now(),?,'Modificación de sitio','Sitios','verSitio.php?id=$id')";
		$q = $pdo->prepare($sql);
            ...
            <div class="row">
              <div class="col-sm-12">
                <div class="card">
				  ...
				  <form class="form theme-form" role="form" method="post" action="modificarSitio.php?id=<?php echo $id?>">
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
							...
							<div class="col-sm-9"><input name="altura" type="number" step="0.01" class="form-control"  value="<?php echo $data['altura']; ?>"></div>
							</div>
							<div class="form-group row">
							<label class="col-sm-3 col-form-label">Ancho Cara</label>
							<div class="col-sm-9"><input name="ancho_cara" type="number" step="0.01" class="form-control"  value="<?php echo $data['ancho_cara']; ?>"></div>
							</div>
							<div class="form-group row">
							<label class="col-sm-3 col-form-label">Peso Estructura</label>
							<div class="col-sm-9"><input name="peso_estructura" type="number" step="0.01" class="form-control"  value="<?php echo $data['peso_estructura']; ?>"></div>
							</div>
							<div class="form-group row">
							...
							<div class="col-sm-9"><input name="paso" type="number" step="0.01" class="form-control"  value="<?php echo $data['paso']; ?>"></div>
							</div>
							<div class="form-group row">
							<label class="col-sm-3 col-form-label">Beta</label>
							<div class="col-sm-9"><input name="beta" type="number" step="0.01" class="form-control"  value="<?php echo $data['beta']; ?>"></div>
							</div>
							<div class="form-group row">
							<label class="col-sm-3 col-form-label">Rugosidad</label>
							<div class="col-sm-9"><input name="rugosidad" type="text" maxlength="99" class="form-control"  value="<?php echo $data['rugosidad']; ?>"></div>
							</div>		
                        </div>
                      </div>
                    </div>
