<?php
    ...
    if (!empty($_POST)) {
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT `id`, `id_packing_list`, `cantidad`, `observaciones` FROM `packing_lists_secciones` WHERE id = ? ";
        $q = $pdo->prepare($sql);
        $q->execute([$id]);
                      ...
                      <div class="row">
                        <div class="col">
						  <div class="form-group row">
							...
							<div class="col-sm-9"><textarea name="observaciones" class="form-control" autofocus required="required"><?php echo $data['observaciones']; ?></textarea></div>
						  </div>
