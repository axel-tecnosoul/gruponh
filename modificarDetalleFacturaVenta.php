<?php
    ...
    if (!empty($_POST)) {
		...
		$sql = "update `facturas_venta_detalle` set `id_concepto_contable` = ?, `cantidad` = ?, `precio` = ?, `subtotal` = ? where id = ?";
		$q = $pdo->prepare($sql);		   
		...
		$sql = " SELECT `cantidad`, `precio`, `subtotal` FROM `facturas_venta_detalle` WHERE `id_factura_venta` = ".$_POST['fv'];    
		...
		$sql = "INSERT INTO logs(`fecha_hora`, `id_usuario`, `detalle_accion`,`modulo`,link) VALUES (now(),?,'Modificación Ítem Detalle de Factura de Venta','Facturas de Venta','')";
		$q = $pdo->prepare($sql);
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT `id`, `id_factura_venta`, id_concepto_contable, `cantidad`, `precio`, `subtotal` FROM `facturas_venta_detalle` WHERE id = ? ";
        $q = $pdo->prepare($sql);
        $q->execute([$_GET['id']]);
        $data = $q->fetch(PDO::FETCH_ASSOC);
		
		$sql2 = "select id_cuenta_destino FROM `facturas_venta` where id = ?";
        $q2 = $pdo->prepare($sql2);
        $q2->execute([$data['id_factura_venta']]);
        $data2 = $q2->fetch(PDO::FETCH_ASSOC);
        $idCuentaCliente = $data2['id_cuenta_destino'];
  ...
  <body>
    ...
    <?php include('menu.php');?>
        ...
        <div class="page-body"><?php
          $ubicacion="Modificar Ítem Detalle de Factura Venta";
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm-12">
				...
				<form class="form theme-form" role="form" method="post" action="modificarDetalleFacturaVenta.php?id=<?php echo $_GET['id'];?>">
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
							...
							foreach ($pdo->query($sql) as $row) {
								...
								$sql2 = " SELECT id from facturas_venta_detalle_x_certificados_avance where id_factura_venta_detalle = ? and id_certificado_avance = ? ";
								$q2 = $pdo->prepare($sql2);
								$q2->execute([$_GET['id'], $row['id']]);
								$data2 = $q2->fetch(PDO::FETCH_ASSOC);
								if (!empty($data2)) {
									echo " selected ";
								}
							...
							<input type="hidden" name="fv" value="<?php echo $data['id_factura_venta'];?>">
                        </div>
                      </div>
                    </div>
