<?php
    ...
    if (!empty($_POST)) {
        ...
        $sql = "UPDATE `computos` set `nro_revision` = `nro_revision` + 1 where id = ?";
        $q = $pdo->prepare($sql);
        $q->execute([$_GET['id']]);
		
		$sql = "INSERT INTO `computos_revisiones`(`id_computo`, `nro_revision`, `comentarios`, `fecha_hora`) VALUES (?,?,?,now())";
		$q = $pdo->prepare($sql);
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT `id`, `nro_revision`, `id_tarea`, `fecha`, `id_cuenta_solicitante`, `aprobado`, `anulado` FROM `computos` WHERE id = ? ";
        $q = $pdo->prepare($sql);
        $q->execute([$id]);
