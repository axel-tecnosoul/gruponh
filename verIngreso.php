<?php
    ...
    if (!empty($_POST)) {
        ...
        $sql = "SELECT `id`, date_format(`fecha_hora`,'%d/%m/%y %H:%i') fecha_hora, `id_tipo_ingreso`, `nro`, `id_cuenta_recibe`, `lugar_entrega`, `observaciones`, date_format(`fecha_remito`,'%d/%m/%Y') fecha_remito, `nro_remito`, `ruta_documento` FROM `ingresos` WHERE id = ? ";
        $q = $pdo->prepare($sql);
        $q->execute([$id]);
...
<html lang="en">
  ...
  <body>
                      ...
                      <div class="row">
                        <div class="col">
                          <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                              <input type="text" class="form-control" value="<?php echo $data['fecha_hora'];?>hs" readonly>
                            </div>
                          </div>
                          <div class="form-group row">
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                              <input type="text" class="form-control" value="<?php echo $data['fecha_remito'];?>" readonly>
                            </div>
                          </div>
                          <div class="form-group row">
                          ...
                          <?php if (!empty($data['ruta_documento'])) { ?>
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Documento Adjunto</label>
                            <div class="col-sm-9">
                              <div class="input-group">
                                <input type="text" id="rutaDocumento" class="form-control" value="<?php echo htmlspecialchars($data['ruta_documento']);?>" readonly="readonly">
                                <div class="input-group-append">
                                  <button type="button" class="btn btn-outline-primary" onclick="copiarRuta()" title="Copiar ruta al portapapeles">
                                    <i class="fa fa-copy"></i> Copiar
                                  </button>
                                </div>
                              </div>
                              <small class="form-text text-muted">Copie la ruta y péguela en el Explorador de Windows para abrir el archivo</small>
                            </div>
                          </div>
                          <?php } ?>
                          <div class="form-group row">
    ...
    function copiarRuta() {
      var rutaInput = document.getElementById('rutaDocumento');
      rutaInput.select();
      rutaInput.setSelectionRange(0, 99999);
      
      navigator.clipboard.writeText(rutaInput.value).then(function() {
        alert('Ruta copiada al portapapeles.\n\nAbra el Explorador de Windows y pegue la ruta (Ctrl+V) en la barra de direcciones.');
      }).catch(function(err) {
