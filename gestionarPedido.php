                      ...
                      <?php if ($data['aprobado'] == 1 && tienePermiso(298)) { ?>
                        <div class="col-md-6">
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                              <select name="id_moneda" id="id_moneda" class="js-example-basic-single w-100" required>
                                ...
                                $q->execute();
                                while ($fila = $q->fetch(PDO::FETCH_ASSOC)) {
                                  $esDolar = (stripos($fila['moneda'], 'dolar') !== false ||
                                    stripos($fila['moneda'], 'dólar') !== false ||
                                    stripos($fila['moneda'], 'usd')   !== false ||
                                    stripos($fila['moneda'], 'u$d')   !== false ||
                                    stripos($fila['moneda'], 'u$s')   !== false) ? 'true' : 'false';
                                  echo "<option value='{$fila['id']}' data-esusd='$esDolar'>{$fila['moneda']}</option>";
                                }
                                Database::disconnect(); ?>
                              </select>
                            </div>
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                              <input name="tipo_cambio_dia" id="tipo_cambio_dia"
  ...
  <script>
    $(document).ready(function() {
          ...
          fetch('https://dolarapi.com/v1/dolares/oficial', {
            ...
            .then(function(d) {
              ...
              var fecha = d.fechaActualizacion ?
                ...
                '';
              info.html(
                'Compra: <strong>$' + parseFloat(d.compra).toLocaleString('es-AR', {
      ...
      function calcularTotalesGenerales() {
        ...
        $('#lbl_neto').text('$ ' + totalNeto.toLocaleString('es-AR', {
          minimumFractionDigits: 2
        }));
        $('#lbl_tasa_iva').text(tasaIva);
        $('#lbl_iva').text('$ ' + montoIva.toLocaleString('es-AR', {
          minimumFractionDigits: 2
        }));
        $('#lbl_total').text('$ ' + totalFinal.toLocaleString('es-AR', {
          minimumFractionDigits: 2
        }));
      }
