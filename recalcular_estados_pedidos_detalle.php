...
try {
  ...
  $sql = "SELECT id FROM pedidos_detalle ORDER BY id";
  ...
  foreach ($registros as $registro) {
    ...
    try {
      ...
      printLog("Recalculando estado para pedido_detalle #$idPedidoDetalle → OK", $isWeb);
      $procesados++;
      
      // Mostrar progreso cada 50 registros
      if ($procesados % 50 == 0) {
        $porcentaje = round(($procesados / $totalRegistros) * 100, 1);
        printLog("Progreso: $procesados/$totalRegistros ($porcentaje%)", $isWeb);
      }
      
    } catch (Exception $e) {
      printLog("ERROR en pedido_detalle #$idPedidoDetalle: " . $e->getMessage(), $isWeb);
      $errores++;
    }
  }
