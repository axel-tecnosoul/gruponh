...
if (!empty($_POST)) {
  ...
  try {
    ...
    foreach ($requiredColumns as $requiredColumn) {
      $sql = "SHOW COLUMNS FROM certificados_maestros_detalles LIKE ?";
      $q = $pdo->prepare($sql);
      $q->execute([$requiredColumn]);
      if (!$q->fetch(PDO::FETCH_ASSOC)) {
        throw new Exception("Falta la columna '" . $requiredColumn . "' en certificados_maestros_detalles. Ejecutar consultas.sql antes de continuar.");
      }
    }
  ...
  $monto_base_lote_anterior = (float) ($loteData['monto_base_lote'] ?? 0);
    ...
    if ($modo_generacion === 'agrupar') {
      ...
      if ($id_lote_edicion !== '' && count($ids_occ_detalle) !== 1) {
        throw new Exception("En edicion de lote modo 'Por cada item' debe seleccionar exactamente un item OCC.");
      }

      foreach ($ids_occ_detalle as $occId) {
        $baseIndividual = (float) ($occSubtotales[$occId] ?? 0);
        $loteAperturado = $id_lote_edicion !== '' ? $loteBase : ($loteBase . '-SEP-' . $occId);
        foreach ($aperturadoRows as $apRow) {
    ...
    <script>
      $(document).ready(function () {
        ...
        function cargarLoteParaEdicion(loteId) {
          ...
          $('html, body').animate({ scrollTop: $('#estado_edicion_lote').offset().top - 80 }, 250);
        }
