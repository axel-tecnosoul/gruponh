<?php
    ...
    if (!empty($_POST)) {
        ...
        $sql = "SELECT pe.`id`, pe.`fecha`, pe.`lugar_entrega`, pe.`id_cuenta_recibe`, pro.nombre, pro.solicitante cuenta_solicitante, pro.nro nro_proyecto, s.nro_sitio, s.id id_sitio, s.nro_subsitio,pro.observaciones FROM `pedidos` pe inner join proyectos pro on pro.id = pe.id_proyecto inner join sitios s on s.id = pro.id_sitio WHERE pe.id = ? ";
		$q = $pdo->prepare($sql);
        $q->execute([$id]);
