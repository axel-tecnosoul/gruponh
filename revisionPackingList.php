...
if (!empty($_POST)) {
  ...
  if($data["nro_revision"]==$nuevo_nro_revision){
    ...
    $q = $pdo->prepare($sql);
    $q->execute([$_POST['fecha'],$_POST['descripcion'],$idCuentaRealizo,$_POST['id_cuenta_reviso'],$_POST['id_cuenta_valido'],$id_packing_list_revision]);
	...
	$sql = "update packing_lists_revisiones set id_estado_packing_list = 7 where id = ?";
    $q = $pdo->prepare($sql);
    $q->execute([$id_packing_list]);
    ...
    $q = $pdo->prepare($sql);
    $q->execute([$data["id_packing_list"],$data['id_proyecto'],$_POST['fecha'],$data['id_usuario'],2,$nuevo_nro_revision,$data['anulado'],$_POST['descripcion'],$idCuentaRealizo,$_POST['id_cuenta_reviso'],$_POST['id_cuenta_valido'],$data['numero']]);
