<?php
	...
	if ( !empty($_POST)) {
		...
		move_uploaded_file($_FILES['adjunto']['tmp_name'],'adjuntos_proyectos/'.$id.'_'.$filename);
			
		$sql = "INSERT INTO `adjuntos_proyecto`(`id_proyecto`, `archivo`, `descripcion`, `anulado`) VALUES (?,?,?,0)";
		$q = $pdo->prepare($sql);
		...
		*/
		...
		$sql = "INSERT INTO logs(`fecha_hora`, `id_usuario`, `detalle_accion`,`modulo`,link) VALUES (now(),?,'Se ha agregado un documento adjunto al proyecto','Proyectos','verProyecto.php?=$id')";
		$q = $pdo->prepare($sql);
		$q->execute(array($_SESSION['user']['id']));
		
		Database::disconnect();
		header("Location: adjuntarProyecto.php?id=".$id);	
...
<html lang="en">
  ...
  <body>
              ...
              <div class="col-sm-12">
                <div class="card">
                  <div class="card-header">
                    <h5>Adjuntar Archivo a Proyecto</h5>
                  </div>
				  <form class="form theme-form" role="form" method="post" action="adjuntarProyecto.php?id=<?php echo $id?>" enctype="multipart/form-data">
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
	...
	<?php
    $pdo = Database::connect();
   $sql = " SELECT `id`, `descripcion`, `archivo`,`id_proyecto` FROM `adjuntos_proyecto` WHERE `anulado` = 0 and `id_proyecto` = ".$_GET['id'];
   foreach ($pdo->query($sql) as $row) {
    ?>
	  <div class="modal fade" id="eliminarModal_<?php echo $row[0]; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
		  ...
		  <a href="eliminarAdjuntoProyecto.php?id=<?php echo $row[0]; ?>&idProyecto=<?php echo $row[3]; ?>" class="btn btn-primary">Eliminar</a>
