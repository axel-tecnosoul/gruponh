...
if (!empty($_POST)) {
  ...
  $sql = "SELECT lc.id AS id_lista_corte, p.nombre nombre_proyecto, p.nro nro_proyecto, s.nro_sitio, s.nro_subsitio, date_format(lc.fecha,'%d/%m/%Y') fecha, lc.id_usuario, lc.id_estado_lista_corte, lc.nro_revision, lc.anulado, lc.nombre, lc.numero, lc.adjunto, cu1.nombre cuenta_elaboro, cu2.nombre cuenta_verifico, cu3.nombre cuenta_aprobo, cu4.nombre nombre_cliente FROM listas_corte lc inner join proyectos p on p.id = lc.id_proyecto inner join sitios s on s.id = p.id_sitio left join cuentas cu1 on cu1.id = lc.id_cuenta_realizo left join cuentas cu2 on cu2.id = lc.id_cuenta_reviso left join cuentas cu3 on cu3.id = lc.id_cuenta_valido inner join cuentas cu4 on cu4.id = p.id_cliente WHERE lc.id = ? ";
  $q = $pdo->prepare($sql);
  $q->execute([$id]);
                      ...
                      <div class="col">
                        <div class="form-group row">
                          ...
                          <div class="col-sm-3 bordered-div">
                            <h6><b><?=$data['nombre']; ?><br>Nº <?=$data['numero']; ?> - Rev <?=$data['nro_revision']; ?><br>Nombre del Plano: <?=$data['adjunto']; ?></b></h6>
                          </div>
                        </div>
                        <div class="form-group row">
                          <div class="col-sm-11 bordered-div">
                            <b>Emisión:</b> <?=$data['fecha'];?><br>
                            <b>Proyecto:</b> <?=$data['nombre_proyecto'];?><br>
                            <b>Cliente:</b> <?=$data['nombre_cliente'];?><br>
                          ...
                          <div class="col-11">
                            <table class="table table-bordered mb-3">
                              ...
                              <tbody><?php
                                ...
                                $sql = " SELECT nro_revision,date_format(fecha,'%d/%m/%y') AS fecha,descripcion,(SELECT c.nombre FROM cuentas c WHERE lc.id_cuenta_realizo=c.id) AS realizo,(SELECT c.nombre FROM cuentas c WHERE lc.id_cuenta_reviso=c.id) AS reviso,(SELECT c.nombre FROM cuentas c WHERE lc.id_cuenta_valido=c.id) AS valido FROM listas_corte lc WHERE lc.id = ".$data['id_lista_corte']." ORDER BY nro_revision ASC";
                          ...
                          <div class="col-sm-12">
                            <table class="display" id="dataTables-example669" border="1" cellspacing="10" cellpadding="5">
                              ...
                              <tbody><?php
                                $pesoTotal = 0;
                                $pdo = Database::connect();
                                $sql = " SELECT lcc.id, lcc.nombre AS nombre_conjunto, lcc.cantidad, lcc.peso, elcc.estado FROM listas_corte_conjuntos lcc inner join estados_lista_corte_conjuntos elcc on elcc.id = lcc.id_estado_lista_corte_conjuntos WHERE lcc.id_lista_corte = ".$id;
                                foreach ($pdo->query($sql) as $row) {
                                  $pesoConjunto = 0;
                                  echo '<tr>';
                                  echo '<td>'. $row["nombre_conjunto"] . '</td>';
                                  ...
                                  <table class="display" border="1" cellspacing="10" cellpadding="5">
                                    ...
                                    <tbody><?php
                                      ...
                                      foreach ($pdo->query($sql2) as $row2) {
                                        ...
                                        echo '<td>'. $row2["largo"] . '</td>';
                                        echo '<td>'. $row2["ancho"] . '</td>';
                                        echo '<td>'. $row2["marca"] . '</td>';
                                        echo '<td>'. $row2["peso_calculado_posicion"] . '</td>';
                                        echo '<td>'. $row2["nro_colada"] . '</td>';
                                        echo '<td>'. $row2["diametro"] . '</td>';
                                        echo '<td>'. $row2["calidad"] . '</td>';
                                        echo '<td>';
                        ...
                        Peso <b>TOTAL</b> de la lista: <?=number_format($pesoTotal,2);?>kgs
                        <div class="col-sm-3 bordered-div">
                          <b>Referencias</b><br>
                          <b>C: </b>Cortar<br>
