                    ...
                    <div class="form-group mb-0">
                      ...
                      <select name="tipo_letra" class="form-control">
                        ...
                        while ($filaTL = $qTL->fetch(PDO::FETCH_ASSOC)) {
                          $selected="";
                          if ($tipo_letra === $filaTL['tipo_letra']){
                            $selected="selected";
                          }?>
                          <option value='<?=$filaTL['tipo_letra']?>' <?=$selected?>><?=$filaTL['tipo_letra']?></option><?php
                        }
                        Database::disconnect();?>
                      </select>
                    </div>
                ...
                <div class="card-header">
                  <h5><?php echo $ubicacion;
                    if (!empty(tienePermiso(336))) { ?>
                      <a href="#" id="btn_nueva_factura_compra" title="Nueva Factura"><img src="img/icon_alta.png" width="24" height="25" border="0" alt="Nueva"></a>&nbsp;&nbsp;<?php
                    } 
                    if (!empty(tienePermiso(338))) {?>
                      <a href="#" id="link_modificar_fc"><img src="img/icon_modificar.png" width="24" height="25" border="0" alt="Modificar/Anular" title="Modificar/Anular"></a>&nbsp;&nbsp;<?php
                    }
                    if (!empty(tienePermiso(338))) {?>
                      <a href="#" id="link_ver_fc"><img src="img/eye.png" width="24" height="15" border="0" alt="Ver Factura" title="Ver Factura"></a>&nbsp;&nbsp;<?php
                    }
                    if (!empty(tienePermiso(336))) {
                    ...
                    if (!empty(tienePermiso(342))) {?>
                      <a href="#" id="link_exportar_fc"><img src="img/xls.png" width="24" height="25" border="0" alt="Exportar" title="Exportar"></a>&nbsp;&nbsp;
                      <a href="#" id="link_exportar_bejerman_fc"><img src="img/import.png" width="24" height="25" border="0" alt="Bejerman TXT" title="Bejerman TXT"></a>&nbsp;&nbsp;<?php
                    }?>
                  </h5>
                </div>
                <div class="card-body">
                  <div class="dt-ext table-responsive">
                    <table class="display truncate" id="dataTables-example666">
                      ...
                      <tbody><?php
                        ...
                        if ($hasSearched){
                          $pdo = Database::connect();
                          $sql = " SELECT fc.id AS id_factura_compra, fc.descripcion, tc.tipo, lc.letra, fc.numero, c.razon_social, date_format(fc.fecha_emitida,'%d/%m/%y') AS fecha_emitida_formatted, fp.forma_pago, fc.total, m.moneda, CASE WHEN fc.id_estado = 3 THEN 'Definitivo' ELSE 'Temporal' END as estado, date_format(fc.fecha_emitida,'%y%m%d') AS fecha_emitida, fc.id_estado, fc.exportada, fc.observaciones FROM facturas_compra fc inner join tipos_comprobante tc on tc.id = fc.id_tipo_comprobante inner join letras_comprobante lc on lc.id = fc.id_letra_comprobante inner join cuentas c on c.id = fc.id_cuenta_origen inner join formas_pago fp on fp.id = fc.id_condicion_pago inner join monedas m on m.id = fc.id_moneda WHERE 1 ";
                          $params = [];
                          ...
                          foreach ($q as $row) {
                            ...
                            <tr data-id-estado="<?=$row['id_estado'];?>" data-exportada="<?=(int)$row['exportada'];?>">
                              <td class="text-center"><input type="checkbox" class="chk-factura" value="<?=$row['id_factura_compra'];?>"></td>
                              <td><?=$row['id_factura_compra'];?></td>
                              <td><?=$row['tipo'] . ' ' . $row['letra'];?></td>
                              <td><?=$row['numero'];?></td>
                              <td><?=$row['razon_social'];?></td>
                              <td><span style="display: none;"><?=$row['fecha_emitida'];?></span><?=$row['fecha_emitida_formatted'];?></td>
                              <td><?=$row['forma_pago'];?></td>
                              <td class="text-right"><?=$row['moneda'] . ' ' . number_format($row['total'] ?? 0,2, ",",".");?></td>
                              <td><?=$row['observaciones'];?></td>
                              <td><?=$row['estado'];?></td>
                              <td class="text-center"><?=$exportada;?></td>
                            </tr><?php
                          }
							            Database::disconnect();
                        }?>
