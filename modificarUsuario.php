<?php
    ...
    if (!empty($_POST)) {
        ...
        $sql = "UPDATE `usuarios` set `usuario`= ?, `id_perfil`= ?, `email`= ?, `activo` = ? where id = ?";
        $q = $pdo->prepare($sql);
        $q->execute([$_POST['usuario'],$_POST['id_perfil'],$_POST['email'],$_POST['activo'],$_GET['id']]);
        
        if (!empty($_POST['clave1'])) {
            $sql = "UPDATE `usuarios` set `contrasenia`= ? where id = ?";
            $q = $pdo->prepare($sql);
            $q->execute([md5($_POST['clave1']),$_GET['id']]);
        }

		$sql = "INSERT INTO logs(`fecha_hora`, `id_usuario`, `detalle_accion`,`modulo`,link) VALUES (now(),?,'Modificación de usuario','Usuarios','verUsuario.php?id=$id')";
		$q = $pdo->prepare($sql);
                      ...
                      <div class="row">
                        <div class="col">
							...
							<div class="form-group row">
								...
								<div class="col-sm-9">
								<select name="id_perfil" id="id_perfil" class="js-example-basic-single col-sm-12" autofocus required="required">
							...
							<div class="form-group row">
								...
								<div class="col-sm-9"><input name="usuario" type="text" maxlength="99" class="form-control" value="<?php echo $data['usuario']; ?>" readonly="readonly"></div>
							</div>
							<div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9"><input name="email" type="email" maxlength="99" value="<?=$data["email"]?>" class="form-control" required="required"></div>
                            </div>
                        </div>
                      </div>
                    </div>
