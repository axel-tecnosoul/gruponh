<?php
    ...
    if (!empty($_POST)) {
        ...
        $sql = "INSERT INTO `listas_corte_detalle_posiciones_procesos`(`id_lista_corte_detalle_posicion`, `id_terminacion`, `detalle_proceso`, `id_cuenta_operario`, `fecha_inicio_proyectada`, `fecha_fin_proyectada`, `fecha_inicio_real`, `fecha_fin_real`, `id_estado_detalle_lista_corte_posiciones_procesos`) VALUES (?,?,?,?,?,?,?,?,?)";
        $q = $pdo->prepare($sql);
        $q->execute([$id,$_POST['id_terminacion'],$_POST['detalle_proceso'],$_POST['id_cuenta_operario'],$_POST['fecha_inicio_proyectada'],$_POST['fecha_fin_proyectada'],$_POST['fecha_inicio_real'],$_POST['fecha_fin_real'],$_POST['id_estado_detalle_lista_corte_posiciones_procesos']]);

        $id = $pdo->lastInsertId();

		$sql = "INSERT INTO logs(`fecha_hora`, `id_usuario`, `detalle_accion`,`modulo`,link) VALUES (now(),?,'Nuevo conjunto ID #$id de Lista de Corte','Listas de Corte','')";
		$q = $pdo->prepare($sql);
        ...
        if (!empty($_POST['btn2'])) {
			...
			header("Location: procesosConjuntosListaCorte.php?id=".$id);	
		}
    }
    ...
    <div class="page-wrapper">
      ...
      <div class="page-body-wrapper">
        ...
        <div class="page-body"><?php
          $ubicacion="Procesos de Conjuntos de Lista de Corte";
            ...
            <div class="row">
              <div class="col-sm-12">
                <div class="card">
				  ...
				  <form class="form theme-form" role="form" method="post" action="procesosConjuntosListaCorte.php?id=<?php echo $id?>">
                    <div class="card-body">
                      <div class="row">
                        <div class="col">
							...
							<table class="display" id="dataTables-example667">
								...
								<tbody>
								  <?php
									$pdo = Database::connect();
									$sql = " SELECT p.`id`, p.`id_lista_corte_detalle_posicion`, t.`terminacion`, p.`detalle_proceso`, cu.`nombre`, date_format(p.`fecha_inicio_proyectada`,'%d/%m/%y'), date_format(p.`fecha_fin_proyectada`,'%d/%m/%y'), date_format(p.`fecha_inicio_real`,'%d/%m/%y'), date_format(p.`fecha_fin_real`,'%d/%m/%y'), e.estado FROM `listas_corte_detalle_posiciones_procesos` p inner join estados_detalle_lista_corte_posiciones_procesos e on e.id = p.`id_estado_detalle_lista_corte_posiciones_procesos` inner join cuentas cu on cu.id = p.`id_cuenta_operario` inner join terminaciones_lista_corte t on t.id = p.id_terminacion WHERE p.id_lista_corte_detalle_posicion = ".$_GET['id'];
									...
									foreach ($pdo->query($sql) as $row) {
										...
										echo '<td>'. $row[9] . '</td>';
										echo '<td>';
										echo '<a href="modificarProcesoConjuntoListaCorte.php?id='.$row[0].'&idLCC='.$row[1].'"><img src="img/icon_modificar.png" width="24" height="25" border="0" alt="Actualizar" title="Actualizar"></a>';
										echo '&nbsp;&nbsp;';
										echo '</td>';
										echo '</tr>';
									}
							...
							<div class="col-sm-9">
							<select name="id_terminacion" id="id_terminacion" class="js-example-basic-single col-sm-12" required="required">
							...
							<div class="col-sm-9"><input name="detalle_proceso" type="text" class="form-control" required="required" value=""></div>
							</div>
							<div class="form-group row">
							...
							<div class="col-sm-9"><input name="fecha_inicio_proyectada" id="fecha_inicio_proyectada" type="date" onfocus="this.showPicker()" value="" class="form-control" ></div>
							</div>
							<div class="form-group row">
							<label class="col-sm-3 col-form-label">FFP</label>
							<div class="col-sm-9"><input name="fecha_fin_proyectada" id="fecha_fin_proyectada" type="date" onfocus="this.showPicker()" value="" class="form-control" ></div>
							</div>
							<div class="form-group row">
							<label class="col-sm-3 col-form-label">FIR</label>
							<div class="col-sm-9"><input name="fecha_inicio_real" id="fecha_inicio_real" type="date" onfocus="this.showPicker()" value="" class="form-control" ></div>
							</div>
							<div class="form-group row">
							<label class="col-sm-3 col-form-label">FFR</label>
							<div class="col-sm-9"><input name="fecha_fin_real" id="fecha_fin_real" type="date" onfocus="this.showPicker()" value="" class="form-control"></div>
							</div>
							<div class="form-group row">
