...
function addMarkersToMap(map) {
	...
	$sql = "SELECT p.`id`, s.`latitud`, s.`longitud`, p.`nombre`, p.`nro`,s.nro_sitio, s.nro_subsitio FROM `proyectos` p 
            INNER JOIN sitios s ON s.id = p.id_sitio 
			inner join cuentas c on c.id = p.id_cliente 
            WHERE p.`anulado` = 0 AND s.`latitud` <> '' AND s.`longitud` <> ''";
    if (!empty($_GET['nro'])) {
    ...
    foreach ($pdo->query($sql) as $row) {
        if ($row[1] != 0) {
    ?>
        var marker = new H.map.Marker({lat: <?php echo $row[1]; ?>, lng: <?php echo $row[2]; ?>});
        marker.setData('<?php echo addslashes($row["nro_sitio"]."-".$row["nro_subsitio"]."-".$row["nro"].": ".$row["nombre"]); ?>');
