...
<html lang="en">
					...
					<form class="form-inline theme-form mt-3" name="form1" method="post" action="listarPackingList.php">
					  ...
					  <div class="form-group mb-0">
						...
						<select name="id_estado[]" id="id_estado[]" class="js-example-basic-multiple" multiple="multiple">
							...
							$sqlZon = "SELECT `id`, `estado` FROM `estados_packing_list` WHERE 1 order by estado ";
              ...
              <div class="col-sm-12">
                <div class="card">
                  <div class="card-header">
                    <h5><?php echo $ubicacion; if (!empty(tienePermiso(348))) { ?><!--<a href="nuevaPackingList.php"><img src="img/icon_alta.png" width="24" height="25" border="0" alt="Nueva" title="Nueva"></a>&nbsp;&nbsp;--><?php } ?>
					...
					echo '&nbsp;&nbsp;';
					echo '<a href="#" id="link_imprimir_pl_con"><img src="img/print.png" width="25" height="20" border="0" alt="Imprimir c/Peso" title="Imprimir c/Peso"></a>';
					echo '&nbsp;&nbsp;';
					echo '<a href="#" id="link_imprimir_pl_sin"><img src="img/print.png" width="25" height="20" border="0" alt="Imprimir s/Peso" title="Imprimir s/Peso"></a>';
					echo '&nbsp;&nbsp;';
                      ...
                      <table class="display truncate" id="dataTables-example666">
                        ...
                        <tbody>
                          <?php 
                            if (!empty($_POST)) {
                            $pdo = Database::connect();
                            $sql = "SELECT pl.id, plr.nro_revision, p.nombre, date_format(plr.fecha,'%d/%m/%y'), e.estado, plr.id, plr.nombre,p.nro, s.nro_sitio, s.nro_subsitio,u.usuario,plr.numero, date_format(plr.fecha,'%y%m%d') FROM packing_lists pl INNER JOIN packing_lists_revisiones plr ON pl.id=plr.id_packing_list inner join proyectos p on p.id = plr.id_proyecto inner join sitios s on s.id = p.id_sitio inner join estados_packing_list e on e.id = plr.id_estado_packing_list inner join usuarios u on u.id = plr.id_usuario WHERE pl.anulado = 0 "; 
							
							if (!empty($_POST['nro'])) {
								$sql .= " and (p.nro = ".$_POST['nro']." or s.nro_sitio = ".$_POST['nro'].") ";
							}
							if (!empty($_POST['fecha'])) {
								$sql .= " AND plr.fecha >= '".$_POST['fecha']."' ";
							}
							if (!empty($_POST['fechah'])) {
								$sql .= " AND plr.fecha <= '".$_POST['fechah']."' ";
							}
                            ...
                            foreach ($pdo->query($sql) as $row) {
                                ...
                                echo '<td>'. $row[6] . '</td>';
								echo '<td>'. $row[1] . '</td>';
								echo '<td><span style="display: none;">'. $row[12] . '</span>'. $row[3] . '</td>';
                                echo '<td>'. $row[10] . '</td>';
                                echo '<td>'. $row[4] . '</td>';
                                echo '</tr>';?>

                                <div class="modal fade" id="eliminarModal_<?=$row[11]?>_<?=$row[5]?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <script>
    ...
    $("#link_imprimir_pl_con").on("click",function(){
			let l=document.location.href;
			if(this.href==l || this.href==l+"#"){
			  alert("Por favor seleccione un packing list para imprimir")
			}
		})
		$("#link_imprimir_pl_sin").on("click",function(){
			let l=document.location.href;
			if(this.href==l || this.href==l+"#"){
			  alert("Por favor seleccione un packing list para imprimir")
			}
    ...
    $(document).on("click","#dataTables-example666 tbody tr td", function(){
        ...
        if(t.hasClass('selected')){
          ...
          $("#link_imprimir_pl_con").attr("href","#");
		  $("#link_imprimir_pl_sin").attr("href","#");
