  ...
  <body>
    ...
    <?php include('menu.php');?>
        ...
        <div class="page-body"><?php
          $ubicacion="Nuevo Componente de Sección";
                  ...
                  <div class="card-header">
                    ...
                    <div class="card-body">
                      <div class="row">
						...
						$pdo = Database::connect();
						$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
						//$sqlZon = "SELECT m.id, m.codigo, m.concepto, cd.reservado from computos_detalle cd inner join materiales m on m.id = cd.id_material inner join computos c on c.id = cd.id_computo inner join tareas t on t.id = c.id_tarea inner join proyectos p on p.id = t.id_proyecto inner join listas_corte lc on lcr.id_proyecto = p.id inner join listas_corte_conjuntos lcc on lcc.id_lista_corte = lc.id where 1 ";
            $sqlZon ="SELECT m.id, m.codigo, m.concepto, cd.reservado from computos_detalle cd inner join materiales m on m.id = cd.id_material inner join computos c on c.id = cd.id_computo inner join tareas t on t.id = c.id_tarea inner join proyectos p on p.id = t.id_proyecto inner join listas_corte_revisiones lcr on lcr.id_proyecto = p.id INNER JOIN listas_corte lc ON lcr.id_lista_corte=lc.id AND lc.ultimo_nro_revision=lcr.nro_revision inner join listas_corte_conjuntos lcc on lcc.id_lista_corte = lcr.id where cd.cancelado = 0 ";
            echo $sqlZon;
